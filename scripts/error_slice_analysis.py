...
def main() -> None:
    ...
    # deterministic example selection (clean gold only)
    nq_ex = sorted(r["id"] for r in census if r["dataset"] == "novelqa" and r.get("abstractors_say_absent"))[:3]
    qa_ex = sorted(r["id"] for r in census if r["dataset"] == "qasper"
                   and r["metric"]["flat"] >= 0.9
                   and r.get("golds") and all(t != "unanswerable" for t, _ in r["golds"]))[:2]
    ...
    ex_lines = ["% Auto-generated by code/scripts/error_slice_analysis.py -- do not edit by hand.",
                ...
                "\\caption{Representative questions where full-context (Flat) is correct while both "
                "summary-based methods fail. On the long novels the summary tree and entity graph report "
                "a present phrase as absent or mis-count it, because the abstraction discarded the wording; "
                "on the short QASPER paper they instead get a document-wide judgement wrong that reading "
                "the whole paper settles. Verbatim model outputs (run~0).}"
                "\\label{tab:results-error-examples}",
                "{\\footnotesize", "\\begin{tabularx}{\\linewidth}{p{0.34\\linewidth}p{0.10\\linewidth}X}",
                "\\toprule", "Question & Gold & Flat / RAPTOR / GraphRAG \\\\", "\\midrule"]
    ...
    for cid in nq_ex:
        ...
        ex_lines.append(f"\\emph{{NovelQA}}: {latex_escape(trunc(r['question'], 90))} & "
                        ...
                        f"{latex_escape(opt(r['answer']['flat']))} / {latex_escape(opt(r['answer']['raptor']))} / "
    ...
    for cid in qa_ex:
        ...
        ex_lines.append(f"\\emph{{QASPER}}: {latex_escape(trunc(r['question'], 90))} & "
                        ...
                        f"{latex_escape(trunc(r['answer']['raptor'], 40))} / "
                        f"{latex_escape(trunc(r['answer']['graphrag'], 40))} \\\\")
